...
def test_run_backtest_with_config_applies_liquidity_bucket_slippage() -> None:
    """Config-driven backtests should pass liquidity-bucket slippage settings."""
    base_config = load_pipeline_config("configs/sample_pipeline.toml")
    assert base_config.backtest is not None
    config = replace(
        base_config,
    ...
    frame["liquidity_bucket"] = ["high", "high", "low"]
...
def test_run_backtest_with_config_applies_row_level_trade_limits() -> None:
    ...
    frame["max_trade_weight"] = [0.0, 0.0, 0.4]
...
def test_run_backtest_with_config_applies_participation_caps() -> None:
    """Config-driven backtests should pass participation caps to the engine."""
    base_config = load_pipeline_config("configs/sample_pipeline.toml")
    assert base_config.backtest is not None
    config = replace(
        base_config,
...
def test_run_backtest_with_config_applies_shortable_column() -> None:
    """Config-driven backtests should pass short availability flags to the engine."""
    base_config = load_pipeline_config("configs/sample_pipeline.toml")
    assert base_config.backtest is not None
    config = replace(
        base_config,
...
def test_run_backtest_with_config_applies_tradable_column() -> None:
    """Config-driven backtests should pass tradability flags to the engine."""
    base_config = load_pipeline_config("configs/sample_pipeline.toml")
    assert base_config.backtest is not None
    config = replace(
        base_config,
