...
- Scoped workflow internals: config-driven data loading helpers, artifact writers, and chart writers live outside the main workflow orchestration module so `alphaforge/cli/workflows.py` does not keep absorbing every reference-data, file-output, or chart-output concern.
