...
- Shared validation core: repeated positive-integer, finite-float, optional/runtime float-bound, numeric-series parsing, scalar/string-series non-empty checks, and required-column checks delegate to `alphaforge.common.validation` while package wrappers preserve their existing exception types.
