...
- Shared validation core: repeated positive-integer, finite-float, optional/runtime float-bound, numeric-series parsing, scalar/string-series non-empty checks, config/runtime scalar choice checks, signal-registry/runtime normalization choices, signal transform sequence parameters, signal score/output column names, portfolio score/weight column names, unique string-sequence / string-pair checks, feature metric-name / metric-pair selections, reference/config field and index selections, and required-column checks delegate to `alphaforge.common.validation` while package wrappers preserve their existing exception types.
- Shared error boundary: public AlphaForge package errors inherit from `alphaforge.common.AlphaForgeError` while preserving `ValueError` compatibility.
...
- Optional `shortable_column` to block negative target weights on rebalance
  dates from explicit row-level short availability flags, such as
...
- `configs/alpha_vantage_us_equities_example.toml`

Most example datasets in `data/raw/` are deterministic and synthetic. They are intended for reproducibility and pipeline inspection, not for claims about tradable alpha.

`configs/alpha_vantage_us_equities_example.toml` points at a small checked-in
Alpha Vantage `TIME_SERIES_DAILY` smoke dataset for AAPL, MSFT, NVDA, XOM, and a
SPY benchmark return series. It is a real-data ingestion example, not a broad
research universe or a basis for alpha claims. API keys should stay in local
environment files such as `.env`, which is ignored by Git.
...
explicit `is_tradable = false` both exclude the row. The joined
