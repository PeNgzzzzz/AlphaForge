...
- Shared validation core: repeated positive-integer, finite-float, optional/runtime float-bound, numeric-series parsing, scalar/string-series non-empty checks, scalar choice checks, unique string-sequence / string-pair checks, feature metric-name / metric-pair selections, reference/config field and index selections, and required-column checks delegate to `alphaforge.common.validation` while package wrappers preserve their existing exception types.
