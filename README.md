...
- Daily OHLCV, benchmark return-series, symbol-metadata, corporate-actions, fundamentals, classifications, and trading-calendar validation with explicit schema, duplicate checks, and conservative integrity rules.
...
- Classifications currently support only effective-date-safe sector/industry histories; they do not yet cover index membership, borrow availability, or more complex reference-data lineage
- Symbol metadata currently covers symbol-level listing/delisting dates only, not index membership or identifier-history workflows
